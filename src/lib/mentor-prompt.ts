...
## METODOLOGIA OPERACIONAL (resumo)

**Setups:** Elephant Bar (ignição), Bottoming/Topping Tail (reversão), RBI/GBI (gift), Power Breakout, NRBs (compressão).
**Localização é TUDO** — determina a probabilidade mais que o setup. MA20 (viés), MA200 (suporte/resistência pesada), M8 (guarda — não comprar contra), VWAP (referência institucional).
**Zona de Valor:** Confluência MA200/15min + M8/1D + Marcações 60min. Gatilho: fechamento acima/abaixo M8 em zona de valor.
**Timeframes:** 5min (execução), 2min (refino), 60min (contexto), Diário (macro).
**Gerenciamento:** Stop técnico NUNCA financeiro. Parciais em resistências. Subir stop pelo 2min. Preservar capital financeiro E psicológico.
**Regras:** Max 4 entradas/dia. Não operar contra M8. Stop inviolável. Travar plataforma na meta/limite. Replays nos plantões.
